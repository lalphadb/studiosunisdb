...
<div class="space-y-6">
    {{-- Header --}}
    <div class="bg-gradient-to-r from-green-500/15 via-green-600/20 to-emerald-500/15 rounded-xl p-6 text-white">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.ceintures.show', $ceinture) }}" 
               class="bg-white/20 hover:bg-white/30 text-white px-6 py-3 rounded-xl font-medium transition duration-200 flex items-center">
        ...
        <form method="POST" action="{{ route('admin.ceintures.attribuer', $ceinture) }}" class="space-y-6">
            ...
            <div>
                ...
                <select name="user_id" id="user_id" required
                        class="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('user_id') border-red-500 @enderror">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="date_obtention" id="date_obtention" required
                           ...
                           max="{{ date('Y-m-d') }}"
                           class="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('date_obtention') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="text" name="examinateur" id="examinateur"
                           ...
                           class="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('examinateur') border-red-500 @enderror">
                    @error('examinateur')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <textarea name="commentaires" id="commentaires" rows="4"
                          ...
                          class="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('commentaires') border-red-500 @enderror">{{ old('commentaires') }}</textarea>
                @error('commentaires')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-between pt-6 border-t border-slate-700/30/30/20>
                <a href="{{ route('admin.ceintures.show', $ceinture) }}" 
                   class="bg-slate-600 hover:bg-slate-700 text-white px-6 py-2 rounded-xl font-medium transition duration-200">
                    Annuler
                </a>
                
                <button type="submit" 
                        class="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-xl font-medium transition duration-200">
    ...
    {{-- Historique récent --}}
    <div class="bg-slate-800/40 backdrop-blur-xl/40 backdrop-blur-xl rounded-xl border border-slate-700/30/30/20p-6">
        <h3 class="text-lg font-semibold text-slate-100 tracking-tight flex items-center gap-3">Attributions Récentes</h3>
        ...
        @if($recent->count() > 0)
            <div class="space-y-3">
                @foreach($recent as $attribution)
                    <div class="flex items-center justify-between p-3 bg-slate-700 rounded-xl">
                        <div class="flex items-center">
