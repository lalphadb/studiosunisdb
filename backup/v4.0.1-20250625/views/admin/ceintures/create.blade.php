...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-yellow-500 to-orange-600 rounded-xl p-6 text-white">
        ...
        <p class="text-yellow-100 text-lg">Attribuer une nouvelle ceinture à un membre</p>
    </div>
        ...
        <form method="POST" action="{{ route('admin.ceintures.store') }}" class="space-y-6">
            ...
            <div>
                ...
                <select name="user_id" id="user_id" required
                        class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('user_id') border-red-500 @enderror">
            ...
            <div>
                ...
                <textarea name="notes" 
                          ...
                          class="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
                @error('notes')
                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
