...
class DashboardController extends Controller
{
    ...
    private function superadminDashboard()
    {
        $stats = [
            ...
            'paiements_mois' => Paiement::whereMonth('created_at', now()->month)->where('statut', 'valide')->sum('montant') ?: 0,
            'seminaires_a_venir' => Seminaire::where('date_debut', '>', now())->count(),
            
            // Top écoles avec protection
    ...
    private function adminDashboard($user)
    {
        ...
        if (!$ecole) {
            return redirect()->route('admin.dashboard')->with('error', 'Aucune école assignée.');
        }
        
        $stats = [
            // Statistiques école sécurisées
            ...
            // Seminaires avec protection
            'seminaires_a_venir' => Seminaire::where('ecole_id', $ecole->id)
                ->where('date_debut', '>', now())
                ->count(),
            
            // Cours populaires avec protection
    ...
    private function instructeurDashboard($user)
    {
        ...
        $stats = [
            ...
            'prochains_seminaires' => Seminaire::where('ecole_id', $ecole->id)
                ->where('date_debut', '>', now())
                ->orderBy('date_debut')
                ->take(3)
                ->get(),
